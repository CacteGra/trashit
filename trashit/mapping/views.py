...
class FirstLoad(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *arg, **kwargs):
        ...
        ts = TrashSpecificities.objects.filter(point_field__o_field__distance_lte=(point,D(m=5000)),from_local_api=True)
        ...
        tt = TrashType.objects.filter(the_type__pk__in=all_types)
        print(tt.count())
        osm_linked_trashes = TrashSpecificities.objects.filter(point_field__o_field__distance_lte=(point,D(m=5000)),osm_trash_spec__isnull=False,from_local_api=True)
        ts = TrashSpecificities.objects.filter(point_field__o_field__distance_lte=(point,D(m=5000)),osm_trash_spec__isnull=True,from_local_api=False).exclude(pk__in=list(osm_linked_trashes.values_list('osm_trash_spec__pk', flat=True)))
        print(ts.count())
        ts = ts.exclude(trash_type__pk__in=tt.values_list('pk', flat=True))
        print(ts.count())
        response, types, not_local_type = self.iterate_points(ts, all_types, not_local_type, request)
