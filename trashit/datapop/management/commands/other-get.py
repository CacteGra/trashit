...
def iterate_child(parent_pk, children_id_list):
...
def get_path(child_id, path_list, first_off):
    ...
    if parent and (parent.chosen.text_chosen != 'root'):
        get_path(parent.id, path_list, False)
    return path_list

def iterate_data_lines(path_list, list_item, l_copy, line_number=0):
    list_item += 1
    n = list_item
        ...
        if (type(path_id) is list):
            ...
            if r.is_list:
                for upper_data_line in l_copy[(r.chosen.text_chosen).replace('[0]', '')]:
                    i = iterate_data_lines(path_list, n-1, upper_data_line, line_number)
                    line_number += 1
                if i:
