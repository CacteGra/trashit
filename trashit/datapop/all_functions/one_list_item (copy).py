...
def only_one_item(using_dict, l, hierarchy_level):
    list_dict = []
    ...
    for i in using_dict:
        ...
        if '[]' in i:
            ...
            if '[0]' in k:
                ...
                follow_list_dict['children'] = only_one_item(sorted(hierarchy(dict_level[0])), dict_level[0], dict_numbered)
            elif isinstance(dict_level[k], dict):
                has_dict = {'dict_done': True, 'name': k}
                follow_list_dict['title'] = k
                follow_list_dict['children'] = only_one_item(sorted(hierarchy(dict_level[k])), dict_level[k], dict_numbered)
            else:
                follow_list_dict['title'] = k
